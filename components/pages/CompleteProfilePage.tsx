...
interface CompleteProfilePageProps {
  email: string;
  onCompleteProfile: (userData: Omit<RegistrationData, 'password' | 'email'>) => Promise<boolean>;
}
...
const CompleteProfilePage: React.FC<CompleteProfilePageProps> = ({ email, onCompleteProfile }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^09\d{8}$/.test(phone)) {
        addToast('error', 'خطأ في البيانات', 'الرجاء إدخال رقم هاتف سوري صالح (مثال: 0912345678).');
        return;
    }
    ...
    await onCompleteProfile({
        ...
        phone,
        governorate,
        avatarUrl: avatarUrl || `https://picsum.photos/seed/${email}/200/200`
    });
  };
      ...
      <div className="max-w-md w-full mx-auto">
        <div className="bg-white rounded-2xl shadow-xl p-8 sm:p-10">
          ...
          <div className="text-center mb-8">
             <h1 className="text-3xl sm:text-4xl font-bold text-slate-800">إكمال الملف الشخصي</h1>
             <p className="text-slate-500 mt-2">نحتاج بعض المعلومات الإضافية لإعداد حسابك.</p>
          </div>
          
          <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                  <label htmlFor="email-display" className={labelStyle}>البريد الإلكتروني</label>
                  <input id="email-display" type="email" value={email} disabled className={`${inputStyle} bg-slate-200 cursor-not-allowed`} />
              </div>
              <div>
